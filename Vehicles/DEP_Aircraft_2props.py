...
def vehicle_setup(cargo_mass=2300*Units.lb, battery_mass=2100*Units.kg):
    ...
    net.number_of_engines = 2. # run various discrete cases
...
def prop_geom(prop):
    prop.tip_radius          = 0.951907 * Units.meter 
...
def prop_origins(prop,net):
    ...
    for i in range(n):
        ...
        if i < n/2: # Left wing from behind
            prop.origin[i,1]  = 1.5 + 1.85*i*prop.tip_radius
            prop.rotation[i]  = -1
        else: # Right wing from behind
            prop.origin[i,1]  = -1.5 - 1.85*(i-n/2)*prop.tip_radius
            prop.rotation[i]  = 1
